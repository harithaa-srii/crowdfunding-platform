...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    if ($_FILES['media']['name'] != "") {
        ...
        $sql_media = "UPDATE logistics_posts SET media=? WHERE id=?";
    ...
    $sql_content = "UPDATE logistics_posts SET content=? WHERE id=?";
    $stmt_content = $conn->prepare($sql_content);
    $stmt_content->bind_param("si", $content, $postId);
    if ($stmt_content->execute()) {
        // Post updated successfully
        echo "<script>alert('Post updated successfully');</script>";
        header("Location: logistics_landing.php"); // Redirect back to the landing page
    } else {
...
if (isset($_GET['post_id'])) {
    ...
    $sql = "SELECT * FROM logistics_posts WHERE id = ?";
...
<div class="back-button">
  <div class="arrow-wrap">
        ...
        document.getElementById('updateForm').addEventListener('submit', function(event) {
            ...
            xhr.onload = function() {
                if (xhr.status === 200) {
                    ...
                    window.location.href = 'logistics_landing.php'; // Redirect to logistics_landing.php after clicking OK
                } else {
    ...
    <script>
            ...
            window.location.href = 'logistics_manage_post.php'; // Adjust the path as needed
            });
